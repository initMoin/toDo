...
- Confirmation that the existing `supabase/` migrations in the Apple project
...
5. From the repository root, copy the Android template:
   ...
   ```text
   ToDo-Android/app/supabase.properties.example
   ```

   to:

   ```text
   ToDo-Android/app/supabase.properties
   ```
...
7. Place it at:
   ...
   ```text
   ToDo-Android/app/google-services.json
   ```
