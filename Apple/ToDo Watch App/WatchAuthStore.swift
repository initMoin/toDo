...
final class WatchAuthStore: ObservableObject {
   ...
   @Published private(set) var prefersUnsignedSession: Bool
   var onLocalAccountBoundaryReset: (() -> Void)?
   ...
   private static let unsignedSessionPreferenceKey = "WatchAuthStore.prefersUnsignedSession"
   ...
   init(userDefaults: UserDefaults = .standard) {
      ...
      prefersUnsignedSession = userDefaults.bool(forKey: Self.unsignedSessionPreferenceKey)
   }

   func start() {
      guard !prefersUnsignedSession else {
         sessionStore.clear()
         authState = .offline
         return
      }
   ...
   func applyPhoneAuthState(_ phoneAuthState: WatchAuthState?) {
      guard !prefersUnsignedSession else { return }
   ...
   func signInWithApple(idToken: String, rawNonce: String, expectedUsername: String) async {
      guard let authClient else {
         errorMessage = String(localized: "toDō Sync is not configured on this Watch.")
         return
      }
      guard let expectedUsername = WatchUsernamePolicy.normalized(expectedUsername) else {
         errorMessage = String(localized: "Use 3–30 letters, numbers, periods, or underscores.")
         return
      }
      ...
      do {
         ...
         guard let profile = try await authClient.fetchProfile(
            accessToken: session.accessToken,
            userID: session.userID
         ), profile.isResolved else {
            errorMessage = String(localized: "Finish account setup on iPhone, Mac, Android, or Web before Watch sync.")
            return
         }
         guard WatchUsernamePolicy.normalized(profile.username ?? "") == expectedUsername else {
            errorMessage = String(localized: "The provider account does not match the username entered. Nothing was linked or moved.")
            return
         }

         try sessionStore.save(session)
         applyResolvedStandaloneSession(session, profile: profile)
      } catch {
   ...
   private func enterUnsignedSession() {
      sessionStore.clear()
      standaloneSession = nil
      authState = .offline
      errorMessage = nil
      prefersUnsignedSession = true
      userDefaults.set(true, forKey: Self.unsignedSessionPreferenceKey)
      onLocalAccountBoundaryReset?()
   }
   ...
   private func applyResolvedStandaloneSession(
      _ session: WatchAuthSession,
      profile: WatchProfileRecord
   ) {
      prefersUnsignedSession = false
      userDefaults.removeObject(forKey: Self.unsignedSessionPreferenceKey)
      standaloneSession = session
      authState = WatchAuthState(
         isAuthenticated: true,
