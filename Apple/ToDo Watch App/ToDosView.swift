...
final class WatchToDoStore: NSObject, ObservableObject, WCSessionDelegate {
   ...
   private var accountBoundaryRevision = 0
   ...
   /// Clears account-scoped presentation state without deleting persisted
   /// records from the phone or backend. Queued actions cannot safely cross an
   /// account boundary because their payload does not carry an account UUID.
   func clearVisibleAccountSnapshot() {
   ...
   func configure(authStore: WatchAuthStore) {
      ...
      authStore.onLocalAccountBoundaryReset = { [weak self] in
         self?.clearVisibleAccountSnapshot()
         self?.requestRefresh()
      }
   ...
   private func refreshDirectly(authSession: WatchAuthSession) {
      ...
      statusText = "Syncing"
      let revision = accountBoundaryRevision
      Task {
         do {
            let remoteItems = try await directSyncClient.fetchToDos(authSession: authSession)
            await MainActor.run {
               guard revision == accountBoundaryRevision,
                     authStore?.standaloneSession?.userID == authSession.userID else { return }
               items = remoteItems
               lastUpdated = .now
   ...
   private func applyDirectly(_ action: WatchToDoAction, authSession: WatchAuthSession) {
      ...
      Task {
         do {
            ...
            await MainActor.run {
               guard revision == accountBoundaryRevision else { return }
   ...
   func mergeQueuedLocalActionsDirectly() async -> Bool {
      ...
      do {
         ...
         let remoteItems = try await directSyncClient.fetchToDos(authSession: authSession)
         guard revision == accountBoundaryRevision,
               authStore?.standaloneSession?.userID == authSession.userID else {
            return false
         }
         actionQueue.clear()
         queuedActionCount = 0
   ...
   private func handle(envelope: WatchEnvelopeParts) {
      ...
      do {
         ...
         case .snapshot:
            ...
            guard let snapshotAuthState = snapshot.authState else {
               // A snapshot without an account boundary cannot be safely
               // attributed after sign-out or an account switch.
               clearVisibleAccountSnapshot()
               statusText = "Connect iPhone"
               return
            }
            if isDifferentAccountBoundary(snapshotAuthState) {
               clearVisibleAccountSnapshot()
            }
            items = snapshot.items
            lastUpdated = snapshot.generatedAt

            authStore?.applyPhoneAuthState(snapshotAuthState)
         ...
         case .authState:
            ...
            if isDifferentAccountBoundary(authState) {
               clearVisibleAccountSnapshot()
            }
   ...
   private func isDifferentAccountBoundary(_ nextState: WatchAuthState) -> Bool {
      guard let currentState = authStore?.authState else { return true }
      return currentState.userID != nextState.userID
         || currentState.isAuthenticated != nextState.isAuthenticated
         || currentState.isAccountResolved != nextState.isAccountResolved
   }
