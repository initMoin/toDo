...
final class WatchConnectivityService: NSObject, ObservableObject {
   ...
   private func createToDo(from action: WatchToDoAction) {
      ...
      do {
         if let cloudID,
            let existingToDo = try context.fetch(FetchDescriptor<ToDo>()).first(where: {
               $0.cloudID == cloudID && $0.ownerUserID == ownerUserID
            }) {
   ...
   private func apply(action: WatchToDoAction) {
      ...
      do {
         ...
         guard let toDo = toDo(matching: action, in: scopedToDos) else {
            send(receipt: WatchToDoActionReceipt(
               actionID: action.id,
               accepted: false,
