...
final class NotificationManager: NSObject, ObservableObject {
         ...
         let schedulableNanoDos = nanoDos
            .filter { nanoDo in
               !nanoDo.isDone
                  ...
                  && nanoDo.toDo.map(isVisibleInCurrentScope(_:)) == true
         ...
         if let nanoDoIdentifier {
            ...
            guard let nanoDo = nanoDos.first(where: {
               persistentIdentifierString(for: $0) == nanoDoIdentifier
                  && $0.toDo.map(isVisibleInCurrentScope(_:)) == true
            }) else {
               return
            }
   ...
   private func isVisibleInCurrentScope(_ toDo: ToDo) -> Bool {
      currentVisibilityScope.includes(
         ownerUserID: toDo.ownerUserID,
         collabID: toDo.collabID
      )
   }

   private var currentVisibilityScope: ToDoVisibilityScope {
      ToDoVisibilityScope(
