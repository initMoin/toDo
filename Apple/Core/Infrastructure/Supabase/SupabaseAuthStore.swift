...
final class SupabaseAuthStore: ObservableObject {
    ...
    var isAuthenticating: Bool {
        authProviderInProgress != nil || isSigningOut
    }
    ...
    var accountDetailText: String {
        ...
        guard isAuthenticated else {
            return String(localized: "Account toDōs stay private and return after you sign in again.")
        }
    ...
    func signOut() async {
        ...
        do {
            ...
            SyncCoordinator.shared.showTransientFeedback(
                title: "Signed Out",
                message: "Account toDōs stay private and return after you sign in again.",
                style: .warning
            )
        } catch {
            // Supabase removes the local session before its remote revocation
            // request. Preserve the successful local privacy boundary when an
            // offline global revocation is the only part that failed.
    ...
    /// Leaves account-backed mode without copying account rows into the local
    /// unsigned namespace. This uses a local-session sign-out so the user can
    /// continue on this device even when the network is unavailable.
    @discardableResult
    func continueWithoutSigningIn() async -> Bool {
        ...
        if SupabaseConfig.isConfigured, activeSession != nil {
            do {
                ...
                // The SDK clears the local session before attempting the
                // server call, so an offline revocation failure is harmless.
            }
        }
    ...
    private func prepareAccountCacheForSignOut(userID: UUID?) async -> Bool {
        guard effectiveSyncMode == .syncEverywhere, let userID else { return true }
        ...
        do {
            ...
            authLog.error("Signed-out account cache cleanup failed: \(String(describing: error), privacy: .public)")
    ...
    private func finishSignedOutBoundary() async {
        session = nil
        currentUser = nil
        profile = nil
        profileImageRevision = 0
