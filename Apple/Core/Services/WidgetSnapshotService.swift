...
struct ToDoWidgetCompletionRequest: Codable, Identifiable, Sendable, Hashable {
   ...
   init(toDoID: String, cloudID: UUID?, ownerUserID: UUID?, completedAt: Date = .now) {
      id = UUID()
      self.toDoID = toDoID
...
final class WidgetSnapshotService {
   ...
   func processPendingCompletionRequests(in context: ModelContext) {
      do {
         ...
         #if os(macOS)
         let currentOwnerUserID = ToDoMacAuthStore.shared.scopedOwnerUserID
         #else
         let currentOwnerUserID = SupabaseAuthStore.shared.scopedOwnerUserID
         #endif
         var handledRequestIDs = Set<UUID>()
         var didChange = false

         for request in requests {
            guard request.ownerUserID == currentOwnerUserID else { continue }
            guard let toDo = toDos.first(where: { candidate in
               guard candidate.ownerUserID == request.ownerUserID else {
                  return false
               }
   ...
   private func makeSnapshot(from context: ModelContext) throws -> ToDoWidgetSnapshot {
      ...
      #if os(macOS)
      let ownerUserID = ToDoMacAuthStore.shared.scopedOwnerUserID
      #else
      let ownerUserID = SupabaseAuthStore.shared.scopedOwnerUserID
      #endif
