...
final class LocationReminderService: NSObject, ObservableObject {
   ...
   private var accessibleCollabIDsProvider: @MainActor () -> Set<UUID> = { [] }
   ...
   func syncMonitoring(for toDos: [ToDo]) {
      ...
      for toDo in toDos where isVisibleInCurrentScope(toDo) && toDo.isActive && toDo.hasLocationReminder {
   ...
   private func isVisibleInCurrentScope(_ toDo: ToDo) -> Bool {
      ToDoVisibilityScope(
         ...
         accessibleCollabIDs: accessibleCollabIDsProvider()
      ).includes(ownerUserID: toDo.ownerUserID, collabID: toDo.collabID)
   }
}
