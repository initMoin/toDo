...
final class MigrationService {
    ...
    func executeIfNeeded(
        ...
        case .deviceOnlyToSyncEverywhere:
            ...
            if shouldTransferData {
                userDefaults.set(userID.uuidString, forKey: AppPreferences.Keys.pendingLocalDataAdoptionUserID)
            } else {
                clearPendingLocalDataAdoption(for: userID, userDefaults: userDefaults)
            }
        case .syncEverywhereToDeviceOnly:
    ...
    func hasPendingLocalDataAdoption(
        for userID: UUID,
        userDefaults: UserDefaults = .standard
    ) -> Bool {
        userDefaults.string(forKey: AppPreferences.Keys.pendingLocalDataAdoptionUserID) == userID.uuidString
    }

    func clearPendingLocalDataAdoption(
        for userID: UUID,
        userDefaults: UserDefaults = .standard
    ) {
        guard hasPendingLocalDataAdoption(for: userID, userDefaults: userDefaults) else { return }
        userDefaults.removeObject(forKey: AppPreferences.Keys.pendingLocalDataAdoptionUserID)
    }
