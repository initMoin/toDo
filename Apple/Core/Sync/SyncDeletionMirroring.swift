...
@MainActor
enum ToDoLocalAccountCacheService {
    /// Clears only the signed-out account's device cache after its final remote
    /// sync succeeds. This intentionally creates no tombstones: the server copy
    /// remains authoritative and will hydrate again on the next sign-in.
    static func clear(
        userID: UUID,
        accessibleCollabIDs: Set<UUID>,
        in context: ModelContext,
        userDefaults: UserDefaults = .standard
    ) throws {
        ...
        let toDos = allToDos.filter {
            shouldRemoveToDo(
                ...
                accessibleCollabIDs: accessibleCollabIDs
            )
        }
        ...
        let nanoDos = allNanoDos.filter { nanoDo in
            nanoDo.ownerUserID == userID
                || nanoDo.toDo.map { toDoIDs.contains($0.id) } == true
        }
        let tags = try context.fetch(FetchDescriptor<Tag>()).filter { $0.ownerUserID == userID }
        let conflicts = try context.fetch(FetchDescriptor<SyncConflict>()).filter { $0.userID == userID }

        do {
            try CalendarIntegrationService.shared.removeCalendarEvents(for: toDos)
        } catch {
            AppLog.error("Sign-out cleanup could not remove every Calendar mirror: \(error)", logger: AppLog.calendar)
        }
        ...
        SyncTombstoneStore.removeAll(for: userID, userDefaults: userDefaults)
        userDefaults.removeObject(forKey: "todo.mac.menuSnapshot")
        NotificationManager.shared.scheduleRefresh()
        WidgetSnapshotService.shared.writeSnapshot(from: context)
    }

    static func shouldRemoveToDo(
        ownerUserID: UUID?,
        collabID: UUID?,
        accountUserID: UUID,
        accessibleCollabIDs: Set<UUID>
        ...
        ownerUserID == accountUserID
            || collabID.map(accessibleCollabIDs.contains) == true
    }
}
