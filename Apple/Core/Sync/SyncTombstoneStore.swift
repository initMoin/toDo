...
private struct SyncTombstoneKey: Hashable {
   let userID: UUID
   let recordTable: SyncRecordTable
   let recordID: UUID
}
...
enum SyncTombstoneStore {
   ...
   static func removeAll(
      ...
      let retained = pendingTombstones(userDefaults: userDefaults).filter { $0.userID != userID }
      save(retained, userDefaults: userDefaults)
   }
   ...
   static func removeSyncedTombstones(
      ...
      let syncedKeys = Set(syncedTombstones.map(\.key))
      ...
      for key in syncedKeys {
         tombstoneDict.removeValue(forKey: key)
      }
...
enum SyncConflictStore {
   static func unresolvedConflicts(in context: ModelContext, userID: UUID?) -> [SyncConflict] {
      ...
      let descriptor = FetchDescriptor<SyncConflict>(
         predicate: #Predicate<SyncConflict> { conflict in
            conflict.resolvedAt == nil && conflict.userID == ownerID
         },
   ...
   static func recordToDoConflict(
      ...
      let conflict = SyncConflict(
         ...
         message: "This toDō changed on more than one device. Choose which version should stay.",
   ...
   static func resolve(
      ...
      case .useSyncedVersion:
         ...
         toDo.lastSyncedUpdatedAt = toDo.updatedAt
         conflict.resolvedAt = .now
      }
   ...
   private static func summary(task: String, dueDate: Date?, isDone: Bool) -> String {
      var parts = [task.isEmpty ? "Untitled toDō" : task]
      if isDone {
         parts.append("Done")
      }
      if let dueDate {
