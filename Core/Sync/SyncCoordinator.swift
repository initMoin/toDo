...
enum SyncOperationPhase: String, Equatable {
    ...
    var title: String {
        ...
        case .uploadingPendingDeletes:
            return "Syncing Deletes"
        case .loadingRemoteChanges:
            return "Checking Account"
        case .cleaningRemoteDuplicates:
            return "Cleaning Duplicates"
        case .applyingRemoteChanges:
    ...
    var detail: String {
        switch self {
        case .activating:
            return "Connecting this device to ToDo Sync."
        case .queuedLocalChanges:
            return "ToDo is batching recent changes before sending them."
        case .preparingLocalData:
            return "Checking local ToDos before syncing."
        case .uploadingPendingDeletes:
            return "Sending delete markers so other devices stay consistent."
        case .loadingRemoteChanges:
            return "Checking your account for newer ToDos."
        case .cleaningRemoteDuplicates:
            return "Removing duplicated synced ToDos safely."
        case .applyingRemoteChanges:
            return "Updating this device with synced ToDos."
        case .sendingLocalChanges:
            return "Sending this device's latest changes."
        case .reconcilingRelationships:
            return "Making sure tags and ToDos stay linked correctly."
        case .listeningForUpdates:
            return "Waiting for changes from your other devices."
        }
    }
}
...
final class SyncCoordinator: ObservableObject {
    ...
    func setPreferredSyncMode(_ mode: SyncMode, userID: UUID?, shouldTransferData: Bool = true) async {
        ...
        if !canApplyWithoutRelaunch(resolvedMode) {
            // CloudKit-backed mode is determined when the SwiftData store is created.
            // Persist the preference now; the UI can surface the restart requirement later.
            showFeedback(
    ...
    private func successMessage(for mode: SyncMode, userID: UUID?, didTransferData: Bool) -> String {
        let transferPrefix = didTransferData ? "" : "ToDo started this sync mode without moving existing ToDos. "
        switch mode {
        case .deviceOnly:
            return "\(transferPrefix)ToDo now keeps what matters on this device."
        case .iCloud:
            return "\(transferPrefix)ToDo now uses iCloud to keep Apple devices in step."
        case .syncEverywhere:
            return userID == nil
                ? "\(mode.title) is selected. Sign in to finish turning it on."
                : "\(transferPrefix)ToDo now uses your account to keep iPhone, Android, and web in step."
        }
    }
}
