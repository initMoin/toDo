...
final class SupabaseSyncService {
   ...
   private func upsertLocalSnapshot(
      ...
      AppLog.info(
         "Preparing \(tagPayloads.count) tag payloads for user \(userID)",
         logger: AppLog.sync
      )
      ...
      if !tagPayloads.isEmpty {
         do {
            ...
            AppLog.info(
               "Tag upsert succeeded.",
               logger: AppLog.sync
            )
         } catch {
            AppLog.error(
               "TAG UPSERT FAILED: \(error)",
               logger: AppLog.sync
            )
            throw error
         }
