...
enum ToDoMacPreferenceKeys {
    ...
    static let menuSnapshot = "todo.mac.menuSnapshot"
...
struct ToDoMacMenuView: View {
    ...
    private var visibleSourceItems: [ToDoMacMenuItem] {
        sourceItems.filter { $0.lifecycleState == .active }
    }
    ...
    private func complete(_ item: ToDoMacMenuItem) {
        ...
        Task { @MainActor in
            ...
            if let toDo = fetchToDo(with: item.id) {
    ...
    private func refreshMenuToDos() {
        ...
        do {
            ...
            let fetchedItems = fetchedToDos.map(ToDoMacMenuItem.init)
    ...
    private func fetchToDo(with id: String) -> ToDo? {
        let descriptor = FetchDescriptor<ToDo>()
        return (try? modelContainer.mainContext.fetch(descriptor))?
            .first { String(describing: $0.id) == id }
    }
...
private struct ToDoMacEditorPane: View {
    ...
    private var scopedOwnerUserID: UUID? {
        SyncCoordinator.shared.effectiveSyncMode == .syncEverywhere ? ToDoMacAuthStore.shared.currentUserID : nil
    }
