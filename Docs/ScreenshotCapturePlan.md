...
For final App Store capture, use a clean simulator state so account prompts, old notification permissions, or prior test data do not leak into screenshots:

```sh
ERASE_SIMULATORS=1 Scripts/capture_app_store_screenshots.sh
```
...
For final Watch screenshots, the same clean-state option is available:

```sh
ERASE_SIMULATORS=1 WATCH_DEVICES="Apple Watch Ultra 3 (49mm)" LOCALES="en,ar,es,hi,it,ja,ms,th,ur,zh-Hans" Scripts/capture_watch_app_store_screenshots.sh
```
